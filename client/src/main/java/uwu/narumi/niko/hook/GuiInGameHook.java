  ...
  public void renderGameOverlay(float partialTicks) {
    ...
    if (!mc.isSingleplayer()) {
      ...
      if (mc.thePlayer.getClientBrand() != null) {
        String brand = mc.thePlayer.getClientBrand().contains("<- ") ? mc.thePlayer.getClientBrand().split(" ")[0] + " -> " + mc.thePlayer.getClientBrand().split("<- ")[1] : mc.thePlayer.getClientBrand().split(" ")[0];
      ...
      if (Holder.getLastPacketMS() != -1) {
        ...
        mc.fontRendererObj
            .drawStringWithShadow(ChatHelper.fix(lastPacketMS > 30500 ? "&fLast packet: &dBroken pipe" : String.format("&fLast packet: &d%s&7ms &fago", packetMs)), 5, 130, 0);
      }
    }
  }
}
